...
def replacer_caselaw(file_data, replacement):
  ...
  """
  replacement_string = f'<ref href="{replacement[3]}" uk:isNeutral="{str(replacement[4]).lower()}" uk:canonical="{replacement[1]}" uk:year="{replacement[2]}" uk:type="case">{replacement[0]}</ref>'
...
def replacer_leg(file_data, replacement):
  ...
  :return: enriched XML file data
  """
  replacement_string = f'<ref href="{replacement[1]}" uk:canonical="{replacement[2]}" uk:type="legislation">{replacement[0]}</ref>'
  file_data = str(file_data).replace(replacement[0], replacement_string)
