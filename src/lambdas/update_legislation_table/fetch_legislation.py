...
def fetch_legislation(sparql_username: str, sparql_password: str, days: int | None) -> pd.DataFrame:
    ...
    log_string += f" since {start_date}" if start_date else ""
...
# currently unused, but could be useful for backfilling legislation data if we want to do it in a single pass rather than incrementally
# have run locally and it works, but not currently used in the lambda
# will discuss with team if we want to keep it or remove it, its a question of simplicity vs speed
def backfill_legislation(
    sparql_username: str,
    sparql_password: str,
    engine,
    table_name: str,
    ...
    for year in range(1800, 2026):
        ...
        SELECT DISTINCT ?ref ?title ?shorttitle ?citation ?acronymcitation
        WHERE {{
            ?ref a leg:Legislation ;
                a leg:UnitedKingdomPublicGeneralAct ;
                leg:title ?title ;
                leg:year {year} ;
                leg:interpretation ?version .
            OPTIONAL {{ ?version leg:shortTitle ?shorttitle }} .
            OPTIONAL {{ ?ref leg:citation ?citation }} .
            OPTIONAL {{ ?ref leg:acronymCitation ?acronymcitation }} .
        }}
        """
        ...
        for col in ["shorttitle", "citation", "acronymcitation"]:
