...
def load_manifest():
    ...
    rules_manifest = pd.read_csv(
        "/Users/daniel.hoadley/Development/project_TNA/rules/2022_02_24_Citation_Manifest.csv"
    )
...
def get_matched_rule_citation_type(rule_id_):
    """Get the citationtype for a given rule_id_"""
    matched_rule = rules_manifest.loc[rules_manifest["id"] == rule_id_]
    return matched_rule["citationType"].iloc[0]


def extract_year_from_wellformed(rule_id_, citation_match):
    """Extracts and returns the year from well-formed citations for
    which the year is a component."""
    NON_YEAR_CITATION_TYPES = [
        "PubNumAbbrNum",
        "PubAbbrNumAbbrNum",
        "EUCCase",
        "EUTCase",
    ]
    citation_type = get_matched_rule_citation_type(rule_id_)
    if citation_type not in NON_YEAR_CITATION_TYPES:
...
def analyse_matched_ids(MATCHED_RULE_ID):
    ...
    df.plot(
        x="rule_name", y=0, kind="barh", figsize=(20, 20), legend=False, color="black"
    )
...
def replacer(file_data, replacement):
    ...
    canonical_form = replacement[1]
    matched_citation = replacement[0]
    year = replacement[2]
    replacement_string = (
        f'<ref type="case" year="{year}" canonical_form="{replacement[1]}">{replacement[0]}</ref>'
    )
    ...
    for file in files[:100]:
        ...
        with open(file_path, "r", encoding="utf-8") as file_in:
            ...
            judgment_content_text = " ".join(
                [content.text for content in judgment_content]
            )
            ...
            for ent in doc.ents:
                ...
                if check_if_canonical(rule_id_) == False:
                    corrected_citation, year = correct_malformed_citation(
                        rule_id_, citation_match
                    )
                    print("-----> CORRECTED:", corrected_citation, file)
                    replacement_entry = (citation_match, corrected_citation, year)
                else:
                    # I don't like what I've done here. DH
                    year = extract_year_from_wellformed(rule_id_, citation_match)
